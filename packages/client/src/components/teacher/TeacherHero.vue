<template>
  <div class="flex flex-col md:flex-row justify-between">
    <div class="flex flex-col md:flex-row gap-10">
      <div class="w-32 h-32 rounded-lg bg-slate-800 flex items-center justify-center text-3xl text-emerald-500">
    ...
    <div class="flex h-max gap-4 flex-col">
      <Button>
        <router-link to="/teacher/list">Înapoi</router-link>
      </Button>
      <Button @click="emit('join')" :variant="isMember || joinLoading ? 'block' : 'primary'">{{
        isMember ? 'Ești înscris' : joinLoading ? 'Loading...' : 'Înscrie-te'
      }}</Button>
    </div>
  </div>
</template>
