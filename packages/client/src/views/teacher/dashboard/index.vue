...
import {
  DayOfWeek,
  type GetTeacherQuery,
  type GetTeacherQueryVariables,
} from '@/api/graphql'
...
const teacherId = computed(() => {
  ...
  const teacherProfile = profiles.find(
    p => p.roleProfile?.__typename === 'Teacher'
  )
...
type TeacherData = NonNullable<
  NonNullable<GetTeacherQuery['teacher']>
>
...
mutation SlotCreate($profileId: ID!, $slot: IAvailabilitySlot!) {
  availabilitySlotCreate(profileId: $profileId, slot: $slot) {
    id dayOfWeek startTime duration
  }
}
`)
...
const saveProfileChanges = async () => {
  ...
  try {
    ...
    /* PROFILE */
    if(profile.profile) await updateProfile({
...
const toggleSlot = async (dayOfWeek: DayOfWeek, startTime: number) => {
  ...
  if (existing) {
    ...
    teacherProfile.value.profile.availabilitySlots =
    ...
    if (res?.data?.availabilitySlotCreate) {
      teacherProfile.value.profile.availabilitySlots?.push(
        res.data.availabilitySlotCreate
      )
    }
  }
}
...
const addStat = () => {
  if(teacherProfile.value?.stats) teacherProfile.value?.stats.push({
    id: "new",
    label: '',
    value: ''
  })
...
const addPlan = () => {
  if(teacherProfile.value?.pricingPlans) teacherProfile.value?.pricingPlans.push({
    id: "new",
    type: 'individual',
    label: '',
    amount: 0,
...
<template>
  <div v-if="loading || meLoading" class="text-center py-10 text-zinc-400">
    Loading...
  </div>
      ...
      <div class="flex-1 flex flex-col gap-6">
        <div class="flex-1 flex flex-col gap-6">
          ...
          <div class="flex gap-4 flex-wrap justify-between">
            ...
            <Input label="Numele de famili" id="profile_secondname" v-model="teacherProfile.profile.lastName" placeholder="Popescu"/>
              ...
              >
                ...
                <Input
                  ...
                  @input="(e: Event) => {
                    ...
                    plan.benefits = target.value
                      ? target.value.split(',')
                      : []
                  }"
                  :id="index + 'plan_benefits'" placeholder="Beneficii (despartite prin virgula)"
                />
        ...
        <TimeSlots
          :availabilitySlots="teacherProfile.profile.availabilitySlots"
          @toggle="toggleSlot"
        />
  ...
  <div v-else class="text-center py-10 text-red-500">
    Forbidden - nu ai profil de teacher
  </div>
</template>
