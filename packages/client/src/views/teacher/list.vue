<template>
  <div class="min-h-screen bg-[#0a0a0b] text-zinc-100 font-sans selection:bg-[#c5a47e]/30">
    <nav
      class="w-full px-6 sm:px-12 py-6 border-b border-white/5 flex justify-between items-center bg-[#0a0a0b]/80 backdrop-blur-md sticky top-0 z-50"
    >
      <div class="flex items-center gap-2">
        ...
        <span class="text-sm font-black uppercase tracking-tighter"
          >alter<span class="text-[#c5a47e]">School</span></span
        >
      </div>
    ...
    <main class="w-full px-4 sm:px-8 md:px-16 lg:px-24 py-16 md:py-24 space-y-16">
      <section class="space-y-6 max-w-4xl">
        ...
        <p class="text-zinc-500 text-lg md:text-xl font-medium leading-relaxed">
          Profesori selectați riguros pentru <span class="text-white">{{ userData.targetCurriculum }}</span
          >, disponibili în intervalele tale.
        </p>
      </section>
      ...
      <section class="grid grid-cols-1 xl:grid-cols-2 gap-8 md:gap-12">
        ...
        >
          <div class="flex flex-col items-center md:items-start gap-6">
            <div class="relative">
              <div
                class="w-32 h-32 md:w-40 md:h-40 rounded-[2rem] bg-zinc-800 border border-white/10 flex items-center justify-center text-4xl font-serif text-[#c5a47e] group-hover:scale-105 transition-transform duration-500"
              >
                {{ teacher.initials }}
              </div>
              <div
                class="absolute -bottom-2 -right-2 bg-green-500 w-5 h-5 rounded-full border-4 border-[#121214]"
              ></div>
            </div>

            <div class="space-y-4 w-full">
              <div
                v-for="stat in teacher.stats"
                :key="stat.label"
                class="flex justify-between md:flex-col md:justify-start gap-1 border-b border-white/5 md:border-0 pb-2 md:pb-0"
              >
            ...
            <div class="space-y-4 text-center md:text-left">
              <div class="flex flex-wrap justify-center md:justify-start gap-2">
                <span
                  ...
                  :key="tag"
                  class="px-2 py-0.5 bg-[#c5a47e]/10 text-[#c5a47e] text-[9px] font-black uppercase rounded border border-[#c5a47e]/20"
                >
                  {{ tag }}
                </span>
              </div>
