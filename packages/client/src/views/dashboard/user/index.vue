...
<template>
  <layout>
    <div class="min-h-screen bg-[#0a0a0b] text-zinc-100 py-10 px-6 font-sans relative">
      <div class="absolute top-0 left-1/2 -translate-x-1/2 w-full h-[600px] bg-[#c5a47e]/[0.02] blur-[140px] pointer-events-none"></div>

<div 
  v-if="loading && !user" 
  class="bg-[#111113] border border-zinc-800/50 rounded-xl overflow-hidden animate-pulse relative z-10"
>
  <div class="p-8 md:p-12 flex flex-col lg:flex-row lg:items-center justify-between gap-8 border-b border-zinc-800/50">
  ...
  <div class="p-12 space-y-12">
    ...
    <div class="h-64 w-full bg-[#111113] border border-zinc-800/50 rounded-xl"></div>
      ...
      <div v-else-if="error" class="bg-red-500/5 border border-red-500/20 p-12 rounded-xl text-center backdrop-blur-md relative z-10">
      ...
      <div v-else-if="user" class="bg-[#111113] border border-zinc-800/50 rounded-xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-700 relative z-10">
